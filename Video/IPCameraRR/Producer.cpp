		...
		media_source *out_source, char *out_name)
{
	...
	format->u.raw_video.display.line_width = fStreamWidth;
	format->u.raw_video.display.line_count = fStreamHeight;

	if (format->u.raw_video.field_rate == 0)
		format->u.raw_video.field_rate = fStreamRate;
...
VideoProducer::FrameGenerator()
{
	...
	while (1) {
		...
		uint32 bufferSize =  bufferWidth * bufferHeight * sizeof(uint32);

		media_header *h = buffer->Header();
		h->type = B_MEDIA_RAW_VIDEO;
		h->time_source = TimeSource()->ID();
		h->size_used = bufferSize;
		...
		h->u.raw_video.line_count = bufferHeight;

		if (fStreamConnected) {
			memcpy((unsigned char*)buffer->Data(),
				(unsigned char*)pFrameRGB->data[0], bufferSize);
			...
			if (fCameraIcon != NULL && fLEDIcon != NULL) {
				...
				BPoint cameraPosition((bufferWidth - fCameraIcon->Bounds().IntegerWidth()) / 2,
					(bufferHeight - fCameraIcon->Bounds().IntegerHeight()) / 2);
				BPoint ledPosition = BPoint(0, 0);

				BPrivate::ConvertBits(fCameraIcon->Bits(), buffer->Data(), fCameraIcon->BitsLength(), bufferSize,
					fCameraIcon->BytesPerRow(), bufferWidth * sizeof(uint32),
				...
				if (inverse) {
					BPrivate::ConvertBits(fLEDIcon->Bits(), buffer->Data(), fLEDIcon->BitsLength(), bufferSize,
						fLEDIcon->BytesPerRow(), bufferWidth * sizeof(uint32),
...
VideoProducer::GetStreamInfo()
{
	...
	fStreamWidth = pCodecCtx->width;
	fStreamHeight = pCodecCtx->height;
	fStreamRate = 29.97f;
	...
	if (fCameraIcon)
		delete fCameraIcon;
	float cameraIconSize = MIN(fStreamWidth, fStreamHeight) * 0.5;
	fCameraIcon = new BBitmap(BRect(0, 0, cameraIconSize, cameraIconSize), B_RGB32);
	BIconUtils::GetVectorIcon(kWebCameraIcon, sizeof(kWebCameraIcon), fCameraIcon);

	if (fLEDIcon)
		delete fLEDIcon;
	float ledIconSize = MIN(fStreamWidth, fStreamHeight) * 0.1;
	fLEDIcon = new BBitmap(BRect(0, 0, ledIconSize, ledIconSize), B_RGB32);
...
VideoProducer::StreamReader()
{
	...
	img_convert_ctx = sws_getContext(pCodecCtx->width, pCodecCtx->height, pCodecCtx->pix_fmt,
		fConnectedFormat.display.line_width + 1, fConnectedFormat.display.line_count + 1,
		AV_PIX_FMT_BGR0, NULL, NULL, NULL, NULL);
